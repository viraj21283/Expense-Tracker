...
import { UserEntity, ChatBoardEntity, ExpenseEntity } from "./entities";
...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
  ...
  app.post('/api/expenses', async (c) => {
    const body = await c.req.json<Omit<Expense, 'id'>>();
    if (typeof body.amount !== 'number' || !isStr(body.description) || !isStr(body.category) || !isStr(body.date)) {
    ...
    await ExpenseEntity.create(c.env, newExpense);
